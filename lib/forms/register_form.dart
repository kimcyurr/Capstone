...
class _RegisterFormState extends State<RegisterForm> {
  ...
  // Temporary user data stored until verification
  ...
  Future<void> registerUser() async {
    ...
    try {
      ...
      // 3️⃣ Prepare temporary user data
      tempUserData = {
  ...
  // Check email verification and create Firestore user
  Future<void> checkEmailVerified() async {
    await currentUser!.reload();
    currentUser = FirebaseAuth.instance.currentUser;

    if (currentUser!.emailVerified) {
      // ✅ Only now save to Firestore
      if (tempUserData != null) {
        await usersRef.doc(currentUser!.uid).set(tempUserData!);
        tempUserData = null; // Clear temporary data
      }
